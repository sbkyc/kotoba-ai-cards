...
describe("AI prompts", () => {
  ...
  it("adds reviewed Chinese meaning and source gloss for JLPT imported English glosses", () => {
    const prompt = buildQuizPrompt(jlptSourceGlossCard);

    expect(prompt).toContain("中文释义: 啊！哦！");
    expect(prompt).toContain("英文释义: Ah!,Oh!");
  });
